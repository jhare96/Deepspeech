...
def load_timit_data(filename):
    audiopath_full = "timit/"+str(filename)+".wav"
    phonemepath_full = "timit/"+str(filename)+".phn"
...
print("number of filepaths", len(timit_filepaths))
windowed_audio_data_list = []
windowed_text_list = []
for i in range(len(timit_filepaths)):
      audio_data, text = load_timit_data(timit_filepaths[i])
      windowed_audio_data_list.append(audio_data)
      windowed_text_list.append(text)


with open(r"processed_timit_audio.pickle", "wb") as output_file:
   pickle.dump(windowed_audio_data_list, output_file)

with open(r"processed_timit_phns.pickle", "wb") as output_file:
